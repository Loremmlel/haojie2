...
import {
  ALL_CELLS, actorCommandError, applyCommand, applyPlayerCommand, canAttemptCommand,
  canDeployKind, canRebasePlayerCommand, commandError, createGame, expansionAnchors,
  getPlayerView, inspectCommand, parseCommand, queryCommandError, template,
...
test('foreign command parsing rejects malformed parameters without touching authority', () => {
  ...
  const invalid: unknown[] = [null, [], {}, { type: 'unknown' }, { type: 'end', rng: 1 },
    { type: 'move', x: NaN }, { type: 'move', x: '3' }, { type: 'move', x: 0 },
    { type: 'attack', path: [{ x: 1, y: 2, secret: 'x' }] },
    { type: 'attack', path: Array(257).fill({ x: 1, y: 1 }) },
    { type: 'summon', ultimate: 1 }, { type: 'choose-shrine', shrineKind: 'not-a-kind' },
    { type: 'skill', unitId: '' }, { type: 'synthesize', materialIds: ['a', 'a'] },
  ...
  assert.deepEqual(parseCommand({ type: 'attack', unitId: 'u1', path: [{ x: 1, y: 1 }, { x: 1, y: 2 }] }).path, [{ x: 1, y: 1 }, { x: 1, y: 2 }]);
});

test('authenticated seat cannot operate an enemy piece even when the rule is out-of-turn', () => {
  const s = fixture(), enemy = add(s, 'u7', 2, 2, 8), target = add(s, 1, 1, 5, 8);
...
test('reaction owner can differ from active and does not grant arbitrary other commands', () => {
  const s = fixture();
  s.pending = [{ kind: 'death-shot', owner: 2, source: template(8, 2, 0, { x: 5, y: 8 }, 'dead-source'), amount: 10 }];
  const before = structuredClone(s);
...
test('both shrine seats may choose independently; spoofed player is rejected and selection reveals together', () => {
  ...
  const first = { type: 'choose-shrine' as const, shrineKind: initial.shrineDraft!.offers[2][0], parity: 'even' as const };
  ...
  const final = applyPlayerCommand(s, 1, { type: 'choose-shrine', shrineKind: s.shrineDraft!.offers[1][0], parity: 'odd' });
  assert.equal(final.phase, 'shrine-setup');
  assert.equal(final.shrineDraft!.revealed, true);
  assert.deepEqual(getPlayerView(final, 1).state.shrineDraft, getPlayerView(final, 2).state.shrineDraft);
});
...
test('unrevealed Jade parity stays private through observation, public logs and public events', () => {
  ...
  const s = applyPlayerCommand(initial, 2, { type: 'choose-shrine', shrineKind: 's9', parity: 'even' });
  s.log.push('SECRET-PARITY-even');
  s.events.push({ id: 'private-debug', type: 'skill', text: 'SECRET-PARITY-even', ability: 's9' });
  const opponent = getPlayerView(s, 1), own = getPlayerView(s, 2);
  ...
  assertNoPrivate(opponent); assertNoPrivate(own);
...
test('nested unit, reaction, clock, effect and event snapshots are explicitly projected and detached', () => {
  const s = fixture(), u = add(s, 1, 1, 4, 5);
  Object.assign(u, { secret: 'not public', rng: 77 });
  u.effects.push(Object.assign({ type: 'attack' as const, from: 1, until: 10, owner: 1 as const, amount: 5 }, { secret: 'effect' }));
  s.pending = [{ kind: 'death-shot', owner: 1, source: u, amount: 10 }];
  s.clockFrames = { 1: { current: { ply: s.ply, turns: s.turns, units: [u] } }, 2: {} };
  s.events = [{ id: 'public-event', type: 'skill', actor: Object.assign({ id: u.id, kind: u.kind, owner: u.owner, size: u.size, x: u.x, y: u.y }, { secret: 'actor' }), from: Object.assign({ x: 4, y: 5 }, { secret: 'coordinate' }) }];
...
test('ordinary public position keeps all present game information except random fields', () => {
  const s = fixture(); add(s, 'u7', 1, 2, 4); add(s, 1, 2, 6, 8);
...
test('public preflight stops before randomness instead of inventing random fields or results', () => {
  const s = createGame(7), view = getPlayerView(s, 1), before = wire(view);
  ...
  const p = fixture(), u = add(p, 9, 1, 4, 5);
  const valid = ALL_CELLS.map((to) => ({ type: 'move' as const, unitId: u.id, ...to })).find((c) => commandError(p, c) === null)!;
  assert.ok(valid);
...
test('local random transitions, turns and deployments are identical through the authority adapter', () => {
  for (const seed of [19, 90, 20260920]) {
    let local = createGame(seed), online = structuredClone(local);
    ...
    for (let turn = 0; turn < 4; turn++) {
      ...
      for (const card of [...local.hands[local.active]]) {
        ...
        const c = ALL_CELLS.map((p) => ({ type: 'deploy' as const, cardId: card.id, ...p })).find((candidate) => commandError(local, candidate) === null);
...
test('host deduplicates the same submission and does not replay stale moves or summons', () => {
  ...
  const first = room.submit(1, request), after = room.inspect();
...
test('same-revision independent secret selections are rebased only by game-owned policy', () => {
  const initial = createGame(90, 'shrine'), room = new DemoRoom('shrine', initial);
  for (const actor of [2, 1] as Player[]) {
    const receipt = room.submit(actor, { requestId: `seat-${actor}`, baseRevision: 0, command: { type: 'choose-shrine', shrineKind: initial.shrineDraft!.offers[actor][0], parity: 'odd' } });
...
test('controlled update selection rejects incompatible versions and ignores equal/older revisions', () => {
  ...
  assert.ok(updateError({ ...one, view: { ...one.view, ruleset: 'wrong' } } as unknown as typeof one));
