...
function resume(s: GameState, actor: Player, command: Command) {
  ...
  for (const viewer of [1, 2] as const) {
    const snapshot: OnlineUpdate = {
      matchId: 'resume-fixture',
      revision: saved.revision,
      kind: 'snapshot',
      view: getPlayerView(saved.state, viewer),
    };
    ...
    const update: OnlineUpdate = {
      ...
      revision: 43,
      kind: 'update',
      view: getPlayerView(next, viewer),
    };
...
test('online resume preserves RNG and next random summon in both modes', () => {
  ...
  const partial = applyPlayerCommand(s, 2, {
    type: 'choose-shrine',
    shrineKind: s.shrineDraft!.offers[2][0],
    parity: 'even',
  });
  const loaded = resume(partial, 1, {
    type: 'choose-shrine',
    shrineKind: partial.shrineDraft!.offers[1][0],
    parity: 'odd',
  });
  ...
  assert.deepEqual(
    getPlayerView(loaded, 2).state.shrineDraft!.choices[2],
    partial.shrineDraft!.choices[2],
  );
});
...
test('online resume rejects mismatched rulesets instead of silently interpreting an old snapshot', () => {
  const update: OnlineUpdate = {
    ...
    revision: 42,
    kind: 'snapshot',
    view: getPlayerView(createGame(1), 1),
  };
