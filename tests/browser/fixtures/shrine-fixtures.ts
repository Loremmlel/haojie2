/** 仅测试使用的夹具通过公开引擎命令构建，不向生产游戏添加测试钩子。 */
...
import { addUnit } from '../../../src/engine/core/state';
import { demolish, captureClockFrame } from '../../../src/engine/setup/shrines';
...
save('flag-card', s);
s = base();
addUnit(s, 's8', 1, { x: 5, y: 7 });
add(s, 1, 1, 5, 7);
save('flag-stack', s);
s = base();
demolish(s, addUnit(s, 's8', 1, { x: 5, y: 7 }) as import('../../../src/engine').Landmark);
save('dormant', s);
s = base();
