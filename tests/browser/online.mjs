...
const bundle = await build({ entryPoints: ['tests/browser/online-host.tsx'], bundle: true, format: 'iife', platform: 'browser', target: 'es2022', jsx: 'automatic', write: false, outfile: 'online-host.js', define: { 'process.env.NODE_ENV': '"development"' } });
...
const browser = await chromium.launch({ executablePath: process.env.BROWSER_PATH || (existsSync('/usr/bin/chromium') ? '/usr/bin/chromium' : undefined), args: ['--no-sandbox', '--disable-dev-shm-usage'] });
...
await context.addInitScript(() => {
  ...
  for (const method of ['getItem', 'setItem', 'removeItem', 'clear']) {
    ...
    Storage.prototype[method] = function (...args) { window.localAccess.push(method); return original.apply(this, args); };
  }
  window.workerStarts = 0;
  window.Worker = class { constructor() { window.workerStarts++; throw new Error('Online client must not start an AI worker'); } };
});
...
async function idle(p = 1) { await status(p).filter({ hasText: '已连接' }).waitFor(); }
async function revision(n) { await page.waitForFunction((n) => window.onlineDemo.revision() === n, n); }
async function summon(p) {
  const before = await page.evaluate(() => window.onlineDemo.revision());
  await button(p, '普通召唤').click(); await revision(before + 1); await idle(p);
}
async function deployAll(p) {
  for (let count = 0; count < 20; count++) {
    ...
    await revision(before + 1); await idle(p);
  }
  assert.fail('fixture deployment loop exceeded its bound');
}
try {
  if (renderOnly) await page.setContent(html); else await page.goto(pathToFileURL(file).href);
  await page.locator('.hj-game').nth(1).waitFor();
  assert.equal(await page.locator('.hj-game').count(), 2);
  for (const name of ['新对局', '悔棋', '重做', '导出存档']) assert.equal(await page.getByRole('button', { name, exact: true }).count(), 0);
  ...
  await summon(1); await summon(1);
  await button(1, '完成召唤，开始行动').click(); await idle(1);
  await deployAll(1);
  await button(1, '结束回合').click(); await idle(1);
  await summon(2); await summon(2);
  await button(2, '完成召唤，开始行动').click(); await idle(2);
  await deployAll(2);
  await button(2, '结束回合').click(); await idle(2);
  assert.equal(await page.evaluate(() => window.onlineDemo.state().active), 1);
  assert.equal(await page.evaluate(() => window.firstBoard === document.querySelector('#player-1 .hj-game')), true);
  const rev = await page.evaluate(() => window.onlineDemo.revision());
  await cell(1, 5, 5).focus(); await page.keyboard.press('Control+z'); await page.keyboard.press('Control+y');
  assert.equal(await page.evaluate(() => window.onlineDemo.revision()), rev);
  check('two JSON-view clients summon, deploy and switch turns without remount or local history controls');

  await reset('combat');
  await page.evaluate(() => window.onlineDemo.configure({ rejectNext: true }));
  await cell(1, 3, 4).click(); await button(1, '攻击').click(); await cell(1, 3, 5).click();
  await client(1).locator('.toast').filter({ hasText: '测试宿主拒绝' }).waitFor();
  await idle(); assert.equal(await page.evaluate(() => window.onlineDemo.revision()), 0);
  await button(1, '攻击').click(); await cell(1, 3, 5).click(); await revision(1); await idle();
  assert.ok((await page.evaluate(() => window.onlineDemo.state().units.find((u) => u.owner === 2 && u.kind === 1).hp)) < 500);
  ...
  await cell(1, 3, 4).click(); await button(1, '攻击').click(); await cell(1, 3, 5).click();
  await revision(1); await status().filter({ hasText: '等待服务器确认' }).waitFor();
  ...
  await page.evaluate(() => window.onlineDemo.flushAcks()); await idle();
  ...
  await page.evaluate(() => window.onlineDemo.flushUpdates()); await idle();
  check('ack arriving first still waits for its authoritative snapshot');

  await reset('classic'); await summon(1);
  ...
  await page.evaluate(() => window.onlineDemo.connect(1, 'connected')); await idle();
  assert.equal(await client(1).locator('.summon-reveal').count(), 0);
  check('duplicate and older updates neither rewind nor replay; reconnect restores a snapshot without summon animation');

  await reset('combat');
  await page.evaluate(() => window.onlineDemo.configure({ holdAck: true, holdUpdates: true }));
  await cell(1, 3, 4).click(); await button(1, '攻击').click(); await cell(1, 3, 5).click(); await revision(1);
  await page.evaluate(() => window.onlineDemo.connect(1, 'disconnected'));
  await status().filter({ hasText: '连接中断' }).waitFor();
  await page.evaluate(() => { window.onlineDemo.flushAcks(); window.onlineDemo.connect(1, 'connected'); });
  await idle();
  assert.ok(await page.evaluate(() => window.onlineDemo.aborted() > 0));
  assert.equal(await page.evaluate(() => window.onlineDemo.revision()), 1);
  check('disconnect aborts waiting, ignores late receipts and reconnects without duplicating a committed command');

  await reset('shrine');
  await page.evaluate(() => window.onlineDemo.configure({ holdUpdates: true }));
  await client(2).locator('.shrine-choice').first().click(); await button(2, '锁定神龛').click(); await revision(1);
  await client(1).locator('.shrine-choice').first().click(); await button(1, '锁定神龛').click(); await revision(2);
  assert.deepEqual(await page.evaluate(() => window.onlineDemo.attempts().map((a) => a.baseRevision)), [0, 0]);
  assert.equal(await page.evaluate(() => window.onlineDemo.state().shrineDraft.revealed), true);
  await page.evaluate(() => { window.onlineDemo.configure({ holdUpdates: false }); window.onlineDemo.flushUpdates(); });
  await idle(1); await idle(2);
  check('both seats choose from the same revision, including non-active side first, and reveal together');

  await reset('reaction');
  assert.ok(await button(1, '放弃此效果').isDisabled());
  assert.ok(await button(2, '放弃此效果').isEnabled());
  await button(2, '放弃此效果').click(); await revision(1); await idle(2);
  ...
  assert.equal(await giant.count(), 1); await giant.click();
  await cell(2, 5, 8).click();
  const destination = await page.evaluate(() => window.onlineDemo.expandCommand());
  await cell(2, destination.x, destination.y).click(); await revision(1); await idle(2);
  assert.equal(await page.evaluate(() => window.onlineDemo.state().units.find((u) => u.owner === 1 && u.x >= 4 && u.y >= 7).size), 2);
  check('out-of-turn BW can target and expand an enemy from the controlled board');

  await reset('path');
  await cell(1, 3, 4).click(); await button(1, '自选穿透路径').click();
  await cell(1, 3, 4).click(); await cell(1, 3, 5).click(); await cell(1, 4, 5).click();
  await button(1, '确认攻击').click(); await revision(1); await idle();
  assert.deepEqual(await page.evaluate(() => window.onlineDemo.attempts()[0].command.path), [{ x: 3, y: 4 }, { x: 3, y: 5 }, { x: 4, y: 5 }]);
  ...
  check('controlled clients start no AI worker, touch no local storage and receive no random/history fields');
  ...
  console.error('Host state:', await page.evaluate(() => window.onlineDemo?.state()).catch(() => null));
  throw error;
} finally {
  await writeFile('artifacts/online-browser-report.json', JSON.stringify({ renderOnly, checks, errors, network }, null, 2));
  await browser.close();
}
