...
try {
  ...
  for (const x of [4, 5, 6]) assert.match(await cell(x, 7).getAttribute('class'), /legal/);
  assert.doesNotMatch(await cell(8, 7).getAttribute('class'), /legal/);
  await cell(5, 7).click();
  assert.equal((await state()).present.landmarks[0].y, 7);
  await button('悔棋').click();
  assert.equal((await state()).present.landmarks.length, 0);
  await button('重做').click();
  check('flag highlights only (4,7)/(5,7)/(6,7) and supports atomic deployment undo/redo');
