...
import {
  applyCommand, applyPlayerCommand, cardActions, commandError, createSession, dispatch,
  getPlayerView, getStats, hitPullDestination, now, other, parseSession, redo, undo, unitActions,
...
function deployable(s: GameState, row: number, expected: boolean) {
  ...
  for (const position of [s, getPlayerView(s, s.active).state]) {
    const a = cardActions(position, position.hands[s.active].find((c) => c.id === id)!)
      .find((a) => a.id === 'deploy')!;
    assert.ok(a);
  ...
  if (expected) assert.ok(applyPlayerCommand(s, s.active, c).units.some((u) => u.x === 8 && u.y === row));
...
for (const owner of [1, 2] as const) {
  test(`feedback4 giant ring: player ${owner} spares friendly/frozen pieces and its base`, () => {
    const s = fixture(); s.active = owner;
  ...
  test(`feedback4 cannon: player ${owner} can select a lane containing only the enemy base`, () => {
    ...
    for (const position of [s, getPlayerView(s, owner).state]) {
      const a = unitActions(position, position.units.find((u) => u.id === cannon.id)!)
        .find((a) => a.id === 'sacrifice')!;
      assert.ok(a);
  ...
  test(`feedback4 deployment: player ${owner} snapshots count differences at turn start`, () => {
    ...
    for (const [own, enemy, expected] of [[1, 1, false], [2, 0, true], [2, 1, false], [3, 1, true]] as const) {
      let s = fixture(); s.active = other(owner);
      for (let i = 0; i < own; i++) add(s, 9, owner, 1 + i * 2, row);
      for (let i = 0; i < enemy; i++) add(s, 9, other(owner), 7, row);
      s = pass(s); s.summonSlots = 0;
...
test('feedback4 row permissions survive movement, save/load and public projection until next own turn', () => {
  const s = fixture();
  add(s, 9, 1, 1, 9); const second = add(s, 9, 1, 3, 9);
  ...
  add(gained, 9, 1, 1, 9); const incoming = add(gained, 9, 1, 3, 8);
...
test('feedback4 all hooks reject giants atomically, including inherited/silenced restrictions and saved pulls', () => {
  for (const kind of [7, 'u23'] as const) for (const silenced of [false, true]) {
    const s = fixture(); const hook = add(s, kind, 1, 3, 4);
    hook.hookReadyAt = now(s, hook); hook.hookExpiresAt = now(s, hook) + 2;
    const big = add(s, 5, 2, 3, 6); big.silenced = silenced;
    const c: Command = { type: 'skill', unitId: hook.id, targetId: big.id, ...(kind === 7 ? { x: 4, y: 4 } : {}) };
    assertAtomic(s, c, /不能被钩子/);
    for (const position of [s, getPlayerView(s, 1).state]) {
      const a = unitActions(position, position.units.find((u) => u.id === hook.id)!)
        .find((a) => a.id === (kind === 7 ? 'hook' : 'superhook'))!;
      assert.ok(a); assert.equal(canChoose(position, startIntent(a), big), false);
    }
    big.kind = 9; big.size = 1; big.traits = [5];
    assertAtomic(s, c, /不能被钩子/);
  }
  const s = fixture(); const hook = add(s, 'formless', 1, 3, 4);
  ...
  s.pending.push({ kind: 'hit-pull', owner: 1, source: structuredClone(hook), targetId: big.id, amount: 0 });
...
test('feedback4 CX can convert an enemy giant but a carrier inheriting giant cannot convert', () => {
  for (const inherited of [false, true]) {
    const s = fixture(); const cx = add(s, 's3', 1, 3, 4);
    const big = add(s, 5, 2, 3, 6);
    if (inherited) cx.traits = [5];
    let rolls = 0;
    const n = applyCommand(s, { type: 'attack', unitId: cx.id, targetId: big.id }, () => { rolls++; return 0; });
