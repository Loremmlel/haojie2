...
for (const p of positions()) {
  ...
  const saved = old.find((r: any) => r.name === p.name && r.seed === 2026092721 && r.budget === 32);
  ...
  for (const seed of protocol.seeds) {
    ...
    const result = bootstrapDecision(p.observation, seed);
    ...
    assert.deepEqual(result, bootstrapDecision(p.observation, seed));
  }
}
...
for await (const r of readTrainingRecords(input)) {
  ...
  for (const seed of protocol.seeds) {
    ...
    for (let i = 0; i < 2 && o.winner === undefined; i++) {
      const result = bootstrapDecision(o, seed);
      ...
      assert.deepEqual(result, bootstrapDecision(o, seed));
      o = next;
    }
...
const summary = {
  ...
  supportedNaturalWins: natural.filter((r) => r.steps[0].result.mode === 'search' && r.value === 1)
    .length,
