...
for (const result of summary.results) {
  ...
  for await (const row of readTrainingRecords(result.output)) {
    ...
    assert.ok(Object.values(s).every((n) => typeof n === 'number' && Number.isFinite(n) && n >= 0));
  ...
  if (lastSearch && !selected.some((r) => r.index === lastSearch.index)) selected.push(lastSearch);
  for (const row of selected) {
    const d = bootstrapDecision(row.observation, row.searchSeed);
