...
首个正式候选为6层Encoder、隐藏维度384、6个注意力头、FFN维度1536，共享策略/价值主干。标准结构主干约10.6M参数，加编码和输出头预计11–13M，最终以实际模型统计为准。早期2–4层、128–256维配置仅用于验证训练闭环。尚未创建、训练或测量这些网络。
...
先导出结构完整的约12M/25M随机权重模型，在目标浏览器测加载、编译预热、batch=1及小批量推理、峰值内存，再投入长期训练。该浏览器模型基准尚未实现，本轮只测Node规则/对弈接口。
...
当前教师仍使用原有启发式候选，不会探索全部路径/组合，也不会自动新增回合外响应窗口。这是热启动基线的策略限制，不是引擎权限限制。未来MCTS需实现分层策略、渐进扩展及响应调度；本轮没有宣称完成MCTS、精确合法掩码、神经网络编码/推理或信息集求解器。
...
本机结果与Node是否足够的判断见[Node性能评估](TRAINING-NODE-2026-09-22.md)。不在CI中用机器耗时写脆弱阈值；行为测试在tests/training，沿用npm test自动运行。
