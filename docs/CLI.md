...
批量训练请使用独立的[常驻训练接口与教师对弈工具](ai/TRAINING.md)。本页的交互CLI继续承担人工操作、保存与复盘；训练工具直接共用底层引擎，不逐步保存Session，也不默认生成评分trace。

网页与CLI的新JSON采用“初始局面＋完整命令历史＋当前局面”，详见[增量存档](session/SAVES.md)。`--replay`同时支持新JSON及原JSONL；`--load`继续兼容旧v2快照。单独新建JSONL记录从当时局面开始，完整既有路线保留在JSON存档中。
