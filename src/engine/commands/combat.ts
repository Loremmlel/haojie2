...
import { availableGuardians, spentGuardSources } from '../core/protection';
import { definition, COMBAT_RULES } from '../catalog';
import { attackProfile, vampireRate } from '../core/attack-profile';
import {
  attackPath,
  piercingTargets,
  validAttackRoute,
...
/** 每个目标的每份效果伤害包独立判断一次护盾；界面纯预览在克隆局面上执行。 */
...
/** 在任何伤害改变占位或杀死目标之前捕获覆盖格。每格独立结算免疫随机、狂怒、名刀和反伤。调用方区分全栈法术与栈顶技能，并明确是否允许友伤。 */
