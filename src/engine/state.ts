...
import { adjacent, cells, other } from './geometry';
import type { Effect, GameEvent, GameState, Kind, Player, Point, Stats, Target, Unit } from './types';

export class RuleError extends Error { constructor(message: string) { super(message); this.name = 'RuleError'; } }
export function ensure(condition: unknown, message: string): asserts condition { if (!condition) throw new RuleError(message); }
...
export function addUnit(s: GameState, kind: Kind, owner: Player, at: Point): Unit {
  ...
  s.units.push(u);
  emit(s,{ type:'spawn', to:at, unitId:u.id, owner },`${faction(owner)}部署了${definition(kind).name}`);
  return u;
}
export function draw(s: GameState, owner: Player, count: number): void {
  for (let i=0; i<count; i++) {
    ...
    const d = definition(kind);
    s.hands[owner].push({ id:`c${s.serial++}`, kind, drawnAt:s.turns[owner], ...(d.spell ? { expiresAt:s.turns[owner]+d.spell } : {}) });
...
export function getStats(s: GameState, u: Unit): Stats {
  ...
  let attack = d.attack + u.attackBonus, range = d.range + u.rangeBonus;
  let actions = u.kind === 4 ? 1 : d.actions, move = d.move;
  const age = s.turns[u.owner]-u.born;
  const sleeping = age <= 0 || (u.kind === 23 && (age < 2 || age % 2 !== 0));
  if (u.kind === '3p') {
    const n = s.units.filter(v => cells(v).some(p => Math.max(Math.abs(p.x-u.x),Math.abs(p.y-u.y)) <= 1)).length;
    attack = Math.max(0,40-5*n)+u.attackBonus; range = n+u.rangeBonus;
  }
  if (u.kind === 4 && u.charge >= 5) range++;
  if (u.kind === 5) move = u.charge > 0 ? 1 : 0;
  if (u.kind === 12 && s.units.some(v => v.owner !== u.owner && adjacent(u,v))) { actions--; move--; }
  attack += u.effects.filter(e => e.type === 'attack' && activeEffect(s,e)).reduce((sum,e) => sum+(e.amount ?? 0),0);
  actions = sleeping ? 0 : Math.max(0,actions);
  return { attack:Math.max(0,attack), range:Math.max(0,range), actions, remaining:Math.max(0,actions-u.spent), move, sleeping };
}
export function findUnit(s: GameState, id?: string): Unit {
  const u = s.units.find(v => v.id === id); ensure(u,'请选择仍在场上的随从。'); return u;
}
...
export function point(x?: number,y?: number): Point { ensure(Number.isInteger(x) && Number.isInteger(y),'请选择棋盘格。'); return { x:x!,y:y! }; }
export function asTarget(u: Unit): Target { return { id:u.id, owner:u.owner, x:u.x, y:u.y, unit:u }; }
export function refreshDeployment(s: GameState, owner: Player): void {
  s.deployRows[owner] = [];
  for (let y=1;y<=13;y++) {
    const count = (p: Player) => s.units.filter(u => u.owner === p && cells(u).some(c => c.y === y)).length;
    if ((owner === 1 ? y<=8 : y>=6) || count(owner)-count(other(owner)) >= 2) s.deployRows[owner].push(y);
  }
}
