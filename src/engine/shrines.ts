/** 3.0 rule-owned mode, landmark and aura operations. No renderer or AI policy here. */
...
import { alive, damage, heal, kill, protectedEffect, type Resolution } from './combat';
import {
  abilityKinds,
  allPieces,
  anyTrait,
...
export function onLandmarkDeployment(s: GameState, u: Unit) {
  ...
  // U17's independent clock progression, not the unrelated normal-17 invulnerability.
  u.offset += 2;
  resetUnit(s, u);
...
export function chooseShrine(s: GameState, c: Command) {
  ...
  // The authoritative state has both commitments. Redacted observations never resolve this.
...
function restoredUnit(s: GameState, u: Unit): Unit {
  ...
  if (old && frame) {
    ...
    // Once-per-real-turn abilities reset through restoration, unlike U17.
...
/** Equipment has one slot. Transfer/return keeps the source card identity to prevent clock duplication. */
...
export function stealOnKill(s: GameState, killer: Unit, victim: Unit) {
  ...
  if (victim.equipment.length && !refusesWeapons(killer)) {
    ...
    // Skills cannot force a weapon whose mage-only rule the new carrier does not satisfy.
    const nextMax =
      killer.maxHp -
    ...
    if (
      ...
      // Qualification belongs to this carrier's wielding period, not the previous one.
