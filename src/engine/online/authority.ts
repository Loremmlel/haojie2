...
/** 解析不可信 JSON，不能只用 TypeScript 类型断言；宿主另行限制传输字节数。 */
export function parseCommand(input: unknown): Command {
  ...
  // 必填和条件参数沿用与本地对局相同的规则函数校验。
  return output as unknown as Command;
}

/** 仅检查操作者权限，允许未完成的界面意图；费用、目标和阶段仍归引擎规则负责。 */
export function actorCommandError(s: GamePosition, actor: Player, c: Command): string | null {
  ...
  // 当前唯一可在回合外执行的棋子命令是免费巨大化；不能使用 active
  // 或 decisionOwner 作为全局联机锁，沉默、冻结、费用和落位由引擎验证。
  if (c.type === 'skill' && unit && (c.ability ?? unit.kind) === 'u7' && hasTrait(unit, 'u7'))
    return null;
  return '当前操作不属于你的席位。';
}

/** 宿主认证操作者、串行处理房间命令，并在回执前持久化结果。 */
