...
const effectsOf = (s: GameState, t: Target): Effect[] =>
...
export function kill(s: GameState, victim: Unit, source?: Unit): void {
  ...
  emit(
    s,
    { type: 'death', to: victim, unitId: victim.id, owner: victim.owner },
    `${faction(victim.owner)}的${definition(victim.kind).name}离场`,
  );
  ...
  if (killer?.kind === 26 && killer.owner !== victim.owner) {
    ...
    emit(
      s,
      { type: 'skill', to: killer, owner: killer.owner, text: '升级' },
      `杀手完成第${killer.kills}次击杀，获得成长`,
    );
  }
  if (victim.kind === 11) s.bonus[victim.owner]++;
  if (victim.kind === 12) addUnit(s, 'grave', victim.owner, victim);
  if (victim.kind === 2)
    s.pending.push({ kind: 'death-shot', owner: victim.owner, source: snapshot, amount: 20 });
...
export function damage(
  ...
  if (!t.unit) {
    ...
    if (loss)
      emit(
        s,
        { type: 'damage', to: t, amount: loss, unitId: t.id, owner: t.owner },
  ...
  if (u.effects.some((e) => e.type === 'immune' && activeEffect(s, e))) {
    emit(
      s,
      { type: 'shield', to: u, unitId: u.id, owner: u.owner, text: '免疫' },
      `${definition(u.kind).name}以金身免疫伤害`,
    );
    return 0;
  }
  ...
  const protector =
    ...
    before <= amount &&
    s.units.find(
      (v) => v.kind === 3 && v.owner === u.owner && attackPath(s, v, t, getStats(s, v).range),
    );
  if (protector) {
    u.guardUsed = true;
    u.hp = 1;
    emit(
      s,
      { type: 'shield', to: u, unitId: u.id, owner: u.owner, text: '名刀' },
      `${definition(u.kind).name}获得名刀保护，以1血存活`,
    );
  } else u.hp = Math.max(0, u.hp - amount);
  const loss = before - u.hp,
    snapshot = structuredClone(u);
  if (loss)
    emit(
      s,
      { type: 'damage', to: u, amount: loss, unitId: u.id, owner: u.owner },
...
export function heal(s: GameState, u: Unit, amount: number): void {
  ...
  emit(
    s,
    { type: 'heal', to: u, amount: gained, unitId: u.id, owner: u.owner },
    `${definition(u.kind).name}回复${gained}生命`,
  );
  if (u.hp === u.maxHp) {
...
export function attack(s: GameState, source: Unit, t: Target, unlimited = false): void {
  const friendly = t.owner === source.owner;
    ...
    !friendly || (t.unit && (source.kind === 2 || (t.unit.kind === 16 && t.id !== source.id))),
    '只能攻击敌方；奶妈可治疗友方，伤害转化器可承受友伤。',
  );
  ...
  emit(
    s,
    {
      ...
      owner: source.owner,
      unitId: source.id,
      text: friendly && source.kind === 2 ? '治疗' : '攻击',
    },
    `${definition(source.kind).name}${friendly && source.kind === 2 ? '治疗' : '攻击'}${t.unit ? definition(t.unit.kind).name : faction(t.owner) + '基地'}`,
  );
  if (friendly && source.kind === 2 && t.unit) {
    heal(s, t.unit, 20);
    return;
  }
  const execute =
    !friendly && t.unit && source.effects.find((e) => e.type === 'execute' && activeEffect(s, e));
  if (execute && t.unit) {
    source.effects = source.effects.filter((e) => e !== execute);
    emit(s, { type: 'skill', from: source, to: t, owner: source.owner, text: '处决' });
    kill(s, t.unit, source);
    return;
  }
  const mark =
    !friendly &&
    source.kind !== 10 &&
    effectsOf(s, t).find(
      (e) => e.type === 'mark' && e.owner === source.owner && activeEffect(s, e),
    );
  ...
  if (source.kind === 10 && !friendly) {
    ...
    if (t.unit ? t.unit.hp === t.unit.maxHp : s.bases[t.owner] === 300) {
      removeEffect(s, t, next);
      loss = damage(s, t, 5, source, path);
    }
  } else {
  ...
  const conversion =
    !friendly && t.unit && source.effects.find((e) => e.type === 'convert' && activeEffect(s, e));
  if (conversion && loss > 0 && t.unit) {
    source.effects = source.effects.filter((e) => e !== conversion);
    if (alive(s, t.unit)) {
      t.unit.owner = source.owner;
      t.unit.born = s.turns[source.owner];
      t.unit.spent = 0;
      t.unit.effects = [];
      t.unit.attacked = [];
      emit(
        s,
        { type: 'skill', to: t.unit, owner: source.owner, text: '策反' },
        `${definition(t.unit.kind).name}加入${faction(source.owner)}`,
      );
    }
  }
}
...
export function react(s: GameState, targetId?: string): void {
  const reaction = s.pending.shift();
  ensure(reaction, '当前没有等待结算的效果。');
  if (!targetId) {
    emit(
      s,
      { type: 'skill', owner: reaction.owner, text: '放弃' },
      `${faction(reaction.owner)}放弃了${reaction.kind === 'death-shot' ? '临终行动' : '伤害转化'}`,
    );
    return;
  }
