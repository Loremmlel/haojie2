...
export function Icon({name,size=18,className=''}:{name:string;size?:number;className?:string}) {
  return <svg className={`icon ${className}`} width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={paths[name] ?? paths.spark}/></svg>;
}
export function Rune({kind,owner=1,large=false}:{kind:Kind;owner?:Player;large?:boolean}) {
  const d=definition(kind);
  return <span className={`rune p${owner} ${d.spell ? 'spell-rune' : ''} ${large ? 'large' : ''}`} aria-hidden="true"><span className="rune-orbit"/><b>{d.glyph}</b><span className="rune-spark">✦</span></span>;
}
export const playerStyle = (p:Player):CSSProperties => ({'--side':p === 1 ? 'var(--teal)' : 'var(--ember)','--side-soft':p === 1 ? 'var(--teal-soft)' : 'var(--ember-soft)'} as CSSProperties);
