...
function bootstrap(initialState: GameState | undefined, storageKey: string | null) {
  ...
  if (storageKey && typeof window !== 'undefined') {
    try {
      ...
      if (saved) return { session: parseSession(saved), notice: '已恢复上次对局。' };
    } catch {
      return {
        session: createSession(createGame(freshSeed())),
        notice: '未能读取旧存档，已创建新对局。你仍可通过“载入”导入之前导出的存档。',
      };
    }
  }
...
export function HaojieGame({
  ...
  const [modal, setModal] = useState<'codex' | 'rules' | 'new' | 'log' | null>(null),
    [notice, setNotice] = useState(boot.notice),
    [events, setEvents] = useState<GameEvent[]>([]);
  const [sound, setSound] = useState(false),
    soundscape = useRef(new Soundscape()),
  ...
  const reaction = s.pending[0],
    inspected = reaction?.source ?? unit;
    ...
    if (storageKey) {
      try {
        ...
        try {
          ...
          setNotice('存储空间不足，只保存了当前局面。仍可导出完整存档。');
        } catch {
          setNotice('浏览器不允许本地保存；请用“导出”保存这盘棋。');
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setEvents([]), 1400);
  ...
  function rewind(forward = false) {
    ...
    const next = forward ? redo(current) : undo(current);
    if (next === current) return;
    replace(next);
    setNotice(forward ? '已重做，随机结果保持不变。' : '已悔棋，手牌、计时与随机数一并恢复。');
  }
  useEffect(() => {
  ...
  function run(command: Command) {
    try {
      const previous = sessionRef.current,
        next = dispatch(previous, command);
      sessionRef.current = next;
      setSession(next);
      setIntent({ kind: 'none' });
      setCardId(null);
      setNotice('');
      setEvents((old) => [...old, ...next.present.events].slice(-65));
      soundscape.current.play(next.present.events, sound);
      if (command.type === 'end') setSelectedId(null);
      if (command.type === 'deploy')
        setSelectedId(next.present.events.find((e) => e.type === 'spawn')?.unitId ?? null);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : '操作未完成，请重试。');
    }
  }
  function onCell(p: Point) {
    const current = sessionRef.current.present;
    if (intent.kind === 'none' && !current.pending.length) {
      setSelectedId(targetAt(current, p)?.id ?? null);
      setCardId(null);
      return;
    }
    if (!canChoose(current, intent, p)) {
      ...
      setNotice(
        command ? (commandError(current, command) ?? '请选择高亮位置。') : '请选择高亮的有效目标。',
      );
      return;
    }
    const command = commandFor(current, intent, p);
    ...
    if (next.kind === 'reforge' && next.targets.length === 2)
      run({ type: 'cast', cardId: next.cardId, mode: 'double', sacrificeIds: next.targets });
    else setIntent(next);
  }
  function chooseCard(id: string) {
    ...
    setNotice('');
    setIntent(
      definition(c.kind).spell
        ? c.kind === 25
          ? { kind: 'none' }
          : { kind: 'cast', cardId: id }
        : { kind: 'deploy', cardId: id, charge: false },
    );
  }
  function start(demo = false) {
    try {
      const seed = seedText.trim() ? Number(seedText) : freshSeed();
      const next = demo ? createDemoGame() : createGame(seed);
      replace(createSession(next));
      setSelectedId(null);
      setModal(null);
      setSeedText('');
      setNotice(
        demo
          ? '演示棋局已载入：选中杀手，攻击超级跑得快，再试试悔棋。'
          : '新对局开始。每人基地300生命，苍穹方先手。',
      );
    } catch (error) {
      setNotice(error instanceof Error ? error.message : '无法开始对局。');
    }
  }
  function exportSave() {
    const blob = new Blob([JSON.stringify(sessionRef.current, null, 2)], {
        type: 'application/json',
      }),
      url = URL.createObjectURL(blob),
      a = document.createElement('a');
    a.href = url;
    a.download = `haojie2-round-${s.ply}.json`;
    a.click();
  ...
  async function importSave(file?: File) {
    if (!file) return;
    try {
      if (file.size > 12_000_000) throw new Error('存档大于12MB，无法载入。');
      const next = parseSession(await file.text());
      replace(next);
      setSelectedId(null);
      setNotice('存档已载入，包含悔棋与重做历史。');
    } catch (error) {
  ...
  function actionMode(
    ...
    if (unit) {
      setIntent({ kind, unitId: unit.id });
      setCardId(null);
      setNotice('');
    }
  }
  const directSkill = (mode?: 'attack' | 'range' | 'charge') =>
    unit && run({ type: 'skill', unitId: unit.id, mode });
  const selectedBase = selectedId === 'base-1' ? 1 : selectedId === 'base-2' ? 2 : null;

  return (
    <div className="hj-game" style={playerStyle(controller)}>
      ...
      <main className="game-container">
            ...
            >
              ...
              <div className="player-score-main">
                ...
                <div className="base-readout">
                  ...
                  <span>/ 300</span>
                  <small>{s.units.filter((u) => u.owner === p).length} 枚在场</small>
                </div>
        ...
        {s.winner && (
          <section className="victory-banner" role="alert">
            ...
            <div>
              <h2>
                {s.winner === 'draw' ? '势均力敌，平局。' : `${faction(s.winner)}，旗开得胜！`}
              </h2>
              <p>基地战结束于第{Math.ceil(s.ply / 2)}轮。也可以悔棋，重新推演这一招。</p>
            </div>
          ...
          <aside className="inspection-rail">
            <section className="panel inspector">
              <div className="panel-heading">
                <h2>{reaction ? '等待效果结算' : card ? '手牌详情' : '棋子情报'}</h2>
                <span>{reaction ? 'REACTION' : card ? 'CARD' : 'INSPECT'}</span>
              </div>
              {d ? (
                <>
                  <div className="inspector-profile">
                    ...
                    <span className="role-chip">
                      {d.role}
                      {card ? ' · 手牌' : ` · ${faction(inspected!.owner)}`}
                    </span>
                    <h3>{d.name}</h3>
                  </div>
                  {inspected && !card && stats ? (
                    <div className="live-stats">
                      {[
                        ['sword', '攻击', stats.attack],
                        ['heart', '生命', `${inspected.hp}/${inspected.maxHp}`],
                        ['target', '射程', stats.range],
                        ['clock', '行动', `${stats.remaining}/${stats.actions}`],
                        ['move', '移动', numberLabel(stats.move)],
                      ].map(([icon, label, value]) => (
                        <div key={label}>
                          <span>
                            <Icon name={String(icon)} size={14} />
                            {label}
                          </span>
                          <b>{value}</b>
                        </div>
                  ...
                  {inspected && !card && (
                    <>
                      <div className="status-tags">
                        {stats?.sleeping && <span>部署疲劳 / 休整中</span>}
                        {inspected.guardUsed && <span>名刀保护已用</span>}
                        {inspected.effects.map((e, i) => (
                          <span key={i} className={e.from > s.ply ? 'upcoming' : ''}>
                            {e.from > s.ply ? '下回合 · ' : ''}
                            {effectNames[e.type]}
                          </span>
                        ))}
                        {inspected.kind === 26 && <span>已击杀 {inspected.kills}</span>}
                        {inspected.kind === 15 && <span>已强化 {inspected.upgrades}/3</span>}
                      </div>
                      {[4, 5, 21].includes(Number(inspected.kind)) && (
                        <div className="charge-meter">
                          <span>
                            {inspected.kind === 5 ? '蓄步' : '蓄力'}{' '}
                            <b>
                              {inspected.charge}/
                              {inspected.kind === 4 ? 5 : inspected.kind === 21 ? 2 : 1}
                            </b>
                          </span>
                          <div>
                            {Array.from(
                              { length: inspected.kind === 4 ? 5 : inspected.kind === 21 ? 2 : 1 },
                              (_, i) => (
                                <i className={i < inspected.charge ? 'filled' : ''} key={i} />
                              ),
                            )}
                          </div>
                        </div>
                  ...
                  {card?.kind === 1 && intent.kind === 'deploy' && (
                    <label className="charge-choice">
                      <input
                        type="checkbox"
                        checked={intent.charge}
                        onChange={(e) => setIntent({ ...intent, charge: e.target.checked })}
                      />
                      <span>
                        <b>以10生命换取冲锋</b>
                        <small>部署后即可行动 · 最大生命变为40</small>
                      </span>
                    </label>
                  )}
                  {card?.kind === 25 && (
                    <div className="reforge-choices">
                      <button
                        className="primary"
                        onClick={() => run({ type: 'cast', cardId: card.id, mode: 'single' })}
                      >
                        <Icon name="spark" />
                        直接召唤一次
                      </button>
                      <button
                        className="secondary"
                        onClick={() => setIntent({ kind: 'reforge', cardId: card.id, targets: [] })}
                      >
                        献祭两枚，召唤两次
                      </button>
                    </div>
                ...
                <div className="base-inspector">
                  <Icon name="crown" size={52} />
                  <h3>{faction(selectedBase)}基地</h3>
                  <strong>
                    {s.bases[selectedBase]} <small>/ 300 HP</small>
                  </strong>
                  <p>这是你必须守护的核心。基地不能被治疗、移动、献祭或策反。</p>
                </div>
              ) : (
                <div className="inspector-empty">
                  ...
                  <h3>选择你的下一步</h3>
                  <p>点击一枚棋子查看能力，或从手牌中选择本回合的豪杰。</p>
                  <button className="text-button" onClick={() => setModal('rules')}>
                    初次对弈？了解规则 <Icon name="arrow" size={15} />
                  </button>
                </div>
              )}
              {unit && !card && !reaction && (
                <div className="unit-actions">
                  ...
                  <div className="action-buttons">
                    <button
                      aria-pressed={intent.kind === 'move'}
                      disabled={!canAct || !stats?.move}
                      onClick={() => actionMode('move')}
                    >
                      <Icon name="move" />
                      移动
                    </button>
                    <button
                      aria-pressed={intent.kind === 'attack'}
                      disabled={!canAct || (unit.kind === 4 && unit.charge < 2)}
                      onClick={() => actionMode('attack')}
                    >
                      <Icon name="sword" />
                      {unit.kind === 4 ? '开炮' : unit.kind === 2 ? '攻击 / 治疗' : '攻击'}
                    </button>
                  </div>
                  {isOwn && (
                    <div className="skill-buttons">
                      {unit.kind === 5 && (
                        <button
                          disabled={!isLegal(s, { type: 'skill', unitId: unit.id })}
                          onClick={() => directSkill()}
                        >
                          <Icon name="spark" />
                          蓄步
                      ...
                      {unit.kind === 6 && (
                        <button disabled={!canAct} onClick={() => directSkill()}>
                          <Icon name="spark" />
                          鼓舞友军
                        </button>
                      )}
                      {unit.kind === 7 && (
                        <button disabled={!canAct} onClick={() => actionMode('hook-target')}>
                          <Icon name="spark" />
                          牵引敌方
                        </button>
                      )}
                      {unit.kind === 14 && (
                        <button
                          disabled={!canAct || unit.maxHp <= 10}
                          onClick={() => actionMode('sacrifice-pick')}
                        >
                          <Icon name="spark" />
                          献祭射击
                        </button>
                      ...
                      {unit.kind === 15 && (
                        <>
                          <button
                            disabled={!canAct || unit.upgrades >= 3}
                            onClick={() => directSkill('attack')}
                          >
                            强化 · +10攻击
                          </button>
                          <button
                            disabled={!canAct || unit.upgrades >= 3}
                            onClick={() => directSkill('range')}
                          >
                            强化 · +1射程
                          </button>
                        </>
                      ...
                      {unit.kind === 19 && (
                        <button disabled={!canAct} onClick={() => actionMode('wall')}>
                          <Icon name="spark" />
                          制造路障
                        </button>
                      )}
                      {unit.kind === 21 && (
                        <>
                          <button
                            disabled={
                              !isLegal(s, { type: 'skill', unitId: unit.id, mode: 'charge' })
                            }
                            onClick={() => directSkill('charge')}
                          >
                            蓄势 {unit.charge}/2
                          </button>
                          <button
                            disabled={
                              !canAct ||
                              unit.charge < 2 ||
                              unit.lastCharge >= s.turns[unit.owner] ||
                              unit.hp <= 10
                            }
                            onClick={() => actionMode('dash-move')}
                          >
                            <Icon name="move" />
                            神行突袭
                          </button>
            ...
            <section className="field-note">
              ...
              <h3>谋定而后动。</h3>
              <p>每一个亮点，是一次选择。移动、攻击与技能共享行动次数。</p>
              <div>
          ...
          <section className="battle-column">
            <div className={`instruction-bar ${reaction ? 'reaction-bar' : ''}`} role="status">
              <span className="instruction-icon">
                <Icon name={reaction ? 'spark' : intent.kind === 'attack' ? 'sword' : 'target'} />
              </span>
              <p>{instruction(s, intent)}</p>
            ...
            <div className="command-bar">
              ...
              {reaction ? (
                <button className="secondary finish-button" onClick={() => run({ type: 'react' })}>
                  放弃此效果 <Icon name="arrow" />
                </button>
              ) : (
                <button
                  ...
                  disabled={!!endError}
                  title={endError ?? '将回合交给对手'}
                >
                  结束回合 <Icon name="arrow" />
                </button>
              )}
            </div>
            <p className="turn-hint">
              {reaction
                ? `现在由${faction(reaction.owner)}处理效果，原回合不会因此结束。`
                : s.winner
                  ? '对局已结束。'
                  : remainingMinions
                    ? `还有 ${remainingMinions} 枚随从需要在本回合部署。`
                    : `可继续行动，或结束回合交给${faction(s.active === 1 ? 2 : 1)}。`}
            </p>
          </section>
          <aside className="hand-rail">
            <section className="panel hand-panel">
              ...
              <p className="hand-intro">
                ...
                {faction(s.active)}
                <span>随从即刻部署 · 法术可储存</span>
              </p>
              <div className="hand-cards">
                {hand.map((c) => {
                  const def = definition(c.kind),
                    left = c.expiresAt !== undefined ? c.expiresAt - s.turns[s.active] : null;
                  return (
                    <button
                      key={c.id}
                      className={`hand-card ${def.spell ? 'spell-card' : ''} ${c.id === cardId ? 'chosen' : ''}`}
                      aria-pressed={c.id === cardId}
                      aria-label={`选择${def.name}${def.spell ? '法术' : '随从'}`}
                    ...
                    >
                      ...
                      <div className="hand-card-main">
                        <div>
                          <h3>{def.name}</h3>
                          <span>{def.spell ? '法术' : def.role}</span>
                        </div>
                        {def.spell ? (
                          <p className={left === 1 ? 'expires-soon' : ''}>
                            <Icon name="clock" size={12} />
                            {left === 1 ? '本回合到期' : `剩余${left}回合`}
                          </p>
                        ) : (
                          <p>
                            <span>
                              <Icon name="sword" size={12} />
                              {c.kind === '3p' ? '动态' : def.attack}
                            </span>
                            <span>
                            ...
                            <span>
                              ...
                              {c.kind === '3p' ? 'n' : def.range}
                            </span>
                          </p>
                        )}
                      </div>
                      {!def.spell && <span className="deploy-label">待部署</span>}
                    </button>
                  );
                })}
                {!hand.length && (
        ...
        <footer className="game-footer">
          ...
          <button onClick={() => setModal('rules')}>规则解释版 1.0</button>
      ...
      {modal === 'log' && (
        <Modal
          title="战场纪事"
          subtitle="按发生顺序记录最近160条事件；悔棋后记录随局面一起恢复。"
      ...
      {modal === 'new' && (
        <Modal
          title="再起一局"
          subtitle="当前对局会被替换。重要对局可先关闭此窗口并导出存档。"
          onClose={() => setModal(null)}
        >
          <div className="new-match-art">
            <Icon name="sword" size={46} />
            <span>9 × 13 · 26种召唤 · 无限可能</span>
          </div>
          <label className="seed-field">
            对局种子 <span>可选，留空随机</span>
            <input
              type="number"
              step="1"
              value={seedText}
              ...
              inputMode="numeric"
              placeholder="例如 20260906"
            />
          </label>
          <button className="primary full-width" onClick={() => start(false)}>
            开始正式对局 <Icon name="arrow" />
          </button>
          <button className="secondary full-width" onClick={() => start(true)}>
            载入演示棋局
          </button>
          <p className="fine-print">
            正式对局从空棋盘开始。演示局预设双方阵型和法术，适合熟悉攻击、技能与悔棋操作。
          </p>
        </Modal>
      )}
    </div>
