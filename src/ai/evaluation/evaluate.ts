...
export function unitValue(
  s: GameState,
  u: Unit,
  valueOf: (v: Unit) => number = (v) => materialValue(s, v),
): number {
  let value = valueOf(u);
...
export function explainEvaluation(s: GameState, side: Player): EvaluationBreakdown {
  ...
  const valueOf = (u: Unit): number => {
    ...
    if (value === undefined) {
      value = materialValue(s, u);
      materials.set(u, value);
    }
    return value;
  };
